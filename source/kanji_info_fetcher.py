...
from source.file_management.file_checker import *
from style.print_colors import bcolors
from source.run_setup import RunSetup
import datetime
import numpy as np
...
class KanjiInfoFetcher():
    ...
    def __init__(self) -> None:     
        ...
        for url in self.url_list:
            ...
            while True:
                ...
                self.fetch_infos()
                self.write_in_csvs()
                self.time_list.append(datetime.datetime.now() - current_time)
                print("Estimated time for conclusion: ", str(np.mean(self.time_list)*(len(self.url_list)-counter)))
    ...
    def fetch_page_content_by_url(self, url : str) -> bytes:
        while True:
            try:
                page = requests.get(url=url, headers=self.headers, timeout=10)
                if page.url == url:
                    return page.content
            except: pass
    ...
    def write_readings_in_csv(self):
        ...
        for line in readings_entry:
            FileWriter().write_entry('data/kanjireadings.csv', line)
    ...
    def alocate_none_values_to_umatched_fields(self):
        ...
        try: self.entry_dict['解説・構成'].remove(self.entry_dict['補足']) 
        except: pass 
    ...
    def write_general_info_in_csv(self):
        ...
        FileWriter().write_entry('data/kanjigeneralinfo.csv', entry)
    ...
    def write_meaning_in_csv(self):
        ...
        for line in meaning_entry:
            FileWriter().write_entry('data/kanjimeaning.csv', line)
    ...
    def write_variant_in_csv(self):
        if self.entry_dict['異体字'] != None and self.entry_dict['異体字'] != 'NULL':
            ...
            for line in variant_forms_entry:
                FileWriter().write_entry('data/kanjivariant.csv', line)
    ...
    def write_misc_in_csv(self):
        if self.entry_dict['解説・構成'] != None and self.entry_dict['解説・構成'] != 'NULL':
            ...
            for line in misc_list_entry:
                FileWriter().write_entry('data/kanjimisc.csv', line)
    ...
    def write_name_readings_in_csv(self):
        if self.entry_dict['名乗り訓'] != None and self.entry_dict['名乗り訓'] != 'NULL':
            ...
            for line in  name_readings_list_entry:
                FileWriter().write_entry('data/kanjinamereadings.csv', line)
    ...
    def write_related_entries_entry_in_csv(self):
        if self.entry_dict['関連項目'] != None and self.entry_dict['関連項目'] != 'NULL':
            ...
            for line in related_kanji_list:
                FileWriter().write_entry('data/relatedkanji.csv', line)
    ...
    def write_in_csvs(self):
        ...
        control_general = entry_was_successfull(self.entry_dict['漢字'], 'data/kanjigeneralinfo.csv', self.general_count)
        if control_general[0] in [1, -1]:
            FileWriter().delete_the_last_x_rows_of_file('data/kanjigeneralinfo.csv', control_general[1])
            self.needs_reprocessing = True
            return
        self.write_meaning_in_csv()
        control_meaning = entry_was_successfull(self.entry_dict['漢字'], 'data/kanjimeaning.csv', self.meaning_count)
        if control_meaning[0] in [1, -1]:
            FileWriter().delete_the_last_x_rows_of_file('data/kanjigeneralinfo.csv', control_general[1])
            FileWriter().delete_the_last_x_rows_of_file('data/kanjimeaning.csv', control_meaning[1])
            self.needs_reprocessing = True
            return
        self.write_variant_in_csv()
        control_variant = entry_was_successfull(self.entry_dict['漢字'], 'data/kanjivariant.csv', self.variant_count)
        if control_variant[0] in [1, -1]:
            FileWriter().delete_the_last_x_rows_of_file('data/kanjigeneralinfo.csv', control_general[1])
            FileWriter().delete_the_last_x_rows_of_file('data/kanjimeaning.csv', control_meaning[1])
            FileWriter().delete_the_last_x_rows_of_file('data/kanjivariant.csv', control_variant[1])
            self.needs_reprocessing = True
            return
        self.write_misc_in_csv()
        control_misc = entry_was_successfull(self.entry_dict['漢字'], 'data/kanjimisc.csv', self.misc_count)
        if control_misc[0] in [1, -1]:
            FileWriter().delete_the_last_x_rows_of_file('data/kanjigeneralinfo.csv', control_general[1])
            FileWriter().delete_the_last_x_rows_of_file('data/kanjimeaning.csv', control_meaning[1])
            FileWriter().delete_the_last_x_rows_of_file('data/kanjivariant.csv', control_variant[1])
            FileWriter().delete_the_last_x_rows_of_file('data/kanjimisc.csv', control_misc[1])
            self.needs_reprocessing = True
            return
        self.write_readings_in_csv()
        control_readings = entry_was_successfull(self.entry_dict['漢字'], 'data/kanjireadings.csv', self.readings_count)
        if control_readings[0] in [1, -1]:
            FileWriter().delete_the_last_x_rows_of_file('data/kanjigeneralinfo.csv', control_general[1])
            FileWriter().delete_the_last_x_rows_of_file('data/kanjimeaning.csv', control_meaning[1])
            FileWriter().delete_the_last_x_rows_of_file('data/kanjivariant.csv', control_variant[1])
            FileWriter().delete_the_last_x_rows_of_file('data/kanjimisc.csv', control_misc[1])
            FileWriter().delete_the_last_x_rows_of_file('data/kanjireadings.csv', control_readings[1])
            self.needs_reprocessing = True
            return
        self.write_name_readings_in_csv()
        control_name_readings = entry_was_successfull(self.entry_dict['漢字'], 'data/kanjinamereadings.csv', self.name_readings_count)
        if control_name_readings[0] in [1, -1]:
            ...
            FileWriter().delete_the_last_x_rows_of_file('data/kanjimisc.csv', control_misc[1])
            FileWriter().delete_the_last_x_rows_of_file('data/kanjireadings.csv', control_readings[1])
            FileWriter().delete_the_last_x_rows_of_file('data/kanjinamereadings.csv', control_name_readings[1])
            self.needs_reprocessing = True
            return
        self.write_related_entries_entry_in_csv()
        control_related = entry_was_successfull(self.entry_dict['漢字'], 'data/relatedkanji.csv', self.related_kanji_count)
        if control_related[0] in [1, -1]:
            ...
            FileWriter().delete_the_last_x_rows_of_file('data/kanjireadings.csv', control_readings[1])
            FileWriter().delete_the_last_x_rows_of_file('data/kanjinamereadings.csv', control_name_readings[1])
            FileWriter().delete_the_last_x_rows_of_file('data/relatedkanji.csv', control_related[1])
            self.needs_reprocessing = True
            return
        self.needs_reprocessing = False
